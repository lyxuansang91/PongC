package com.hungle.pongc.screens.menu;
...
import com.hungle.pongc.Tweens.Value;
import com.hungle.pongc.Tweens.ValueAccessor;
import com.hungle.pongc.helpers.AssetLoader;
import com.hungle.pongc.helpers.ColorManager;
...
public class Pad {
    ...
    public Pad(MenuWorld gameWorld) {
    ...
    public void update(float delta) {
        ...
        angVel += angAcc * delta;
        angle += angVel * delta;
        distanceToCenter = world.getMenuObject().getMenuButtons().get(0).getBounds().getWidth()/2;
    ...
    public void render(SpriteBatch batch, ShapeRenderer shapeRenderer) {
        ...
        shapeRenderer.end();
        batch.begin();
        sprite.setColor(ColorManager.parseColor("#2c3e50",1f));
        sprite.draw(batch);
    ...
    private Vector2 calculatePosition(int i) {
        ...
        float cy = world.gameHeight / 2+90;
    ...
    public void end(){
        Tween.to(dtcVal, -1, .5f).target(0).repeatYoyo(0, 0)
    ...
    public void start(){
        ...
        Tween.to(spriteSize, -1, 1f).target(0.5f).repeatYoyo(0, 0)
                .ease(TweenEquations.easeOutSine).start(manager);
    }
